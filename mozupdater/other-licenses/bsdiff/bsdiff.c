...
int main(int argc,char *argv[])
{
	...
	if(((fd=_open(argv[2],O_RDONLY|_O_BINARY,0))<0) ||
		...
		(_lseek(fd,0,SEEK_SET)!=0) ||
		(_read(fd,newbuf,newsize)!=newsize) ||
		(_close(fd)==-1)) reporterr(1,"%s\n",argv[2]);
	...
	if((fd=_open(argv[3],O_CREAT|O_TRUNC|O_WRONLY|_O_BINARY,0666))<0)
		reporterr(1,"%s\n",argv[3]);
	...
	if(_write(fd,&header,sizeof(MBSPatchHeader))!=sizeof(MBSPatchHeader))
		reporterr(1,"%s\n",argv[3]);
	...
	while(scan<newsize) {
		...
		if((len!=oldscore) || (scan==newsize)) {
			...
			if (_write(fd,&triple,sizeof(triple)) != sizeof(triple))
				reporterr(1,NULL);
	...
	if(_write(fd,db,dblen)!=dblen)
		reporterr(1,NULL);

	if(_write(fd,eb,eblen)!=eblen)
		reporterr(1,NULL);
	...
	if (_lseek(fd,0,SEEK_SET) == -1 ||
	    _write(fd,&header,sizeof(header)) != sizeof(header) ||
	    _close(fd) == -1)
		reporterr(1,NULL);
