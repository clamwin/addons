...
#ifdef XP_WIN
extern int RenameFileOnReboot(const char *source, const char *dest);
...
static int copy_file(const char *spath, const char *dpath)
{
  ...
  AutoFD sfd = _open(spath, O_RDONLY | _O_BINARY);
  if (sfd < 0 || fstat(sfd, &ss)) {
    LOG(("copy_file: failed to open or stat: %d,%s,%d\n", (int) sfd, spath, errno));
    return READ_ERROR;
  }

  AutoFD dfd = _open(dpath, O_WRONLY | O_TRUNC | O_CREAT | _O_BINARY, ss.st_mode);
  if (dfd < 0) {
  ...
  char buf[BUFSIZ];
  int sc;
  while ((sc = _read(sfd, buf, sizeof(buf))) > 0) {
    int dc;
    char *bp = buf;
    while ((dc = _write(dfd, bp, (unsigned int) sc)) > 0) {
      if ((sc -= dc) == 0)
        break;
      bp += dc;
    }
...
PatchFile::LoadSourceFile(int ofd)
{
  ...
  while (r) {
    int c = _read(ofd, rb, mmin(BUFSIZ,r));
    if (c < 0)
      return READ_ERROR;
...
PatchFile::Prepare()
{
  ...
  pfd = _open(spath, O_RDONLY | _O_BINARY);
  if (pfd < 0)
    return READ_ERROR;
  ...
  AutoFD ofd = _open(mFile, O_RDONLY | _O_BINARY);
  if (ofd < 0)
    return READ_ERROR;
...
PatchFile::Execute()
{
  ...
  char backup[MAXPATHLEN];
  snprintf(backup, sizeof(backup), "%s" BACKUP_EXT, mFile);
  ...
  int ofd = _open(backup, O_WRONLY | O_TRUNC | O_CREAT | _O_BINARY, ss.st_mode);
  if (ofd < 0)
    return WRITE_ERROR;

  rv = MBS_ApplyPatch(&header, pfd, buf, ofd);
  _close(ofd);
  if (rv)
  {
      remove(backup);
  ...
  rv = rename(backup, mFile);
   ...
   if (rv)
   {
       LOG(("REBOOT NEEDED %s\n", mFile));
       RebootNeeded = true;
       return RenameFileOnReboot(backup, mFile);
   }
#else
  if (rv) remove(backup);
#endif
...
WriteStatusFile(int status)
{
  ...
  AutoFD fd = _open(filename, O_WRONLY | O_TRUNC | O_CREAT | _O_BINARY, 0644);
  if (fd < 0)
    return;
  ...
  _write(fd, text, (PRUint32) strlen(text));
}
...
int main(int argc, char **argv)
{
  ...
  if (RebootNeeded) putenv("REBOOT_NEEDED=1");
#endif
...
int DoUpdate()
{
  ...
  AutoFD mfd = _open(manifest, O_RDONLY | _O_BINARY);
  if (mfd < 0)
    return READ_ERROR;
  ...
  char *rb = mbuf;
  while (r) {
    int c = _read(mfd, rb, mmin(SSIZE_MAX,r));
    if (c < 0)
      return READ_ERROR;
